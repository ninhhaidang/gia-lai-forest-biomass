 ...
 * sử dụng Gradient Tree Boosting trên nền tảng Google Earth Engine
 * 
 * Mã nguồn này thực hiện việc phân tích và ước tính sinh khối rừng trên mặt đất (AGB)
 * cho tỉnh Gia Lai, Việt Nam, sử dụng thuật toán Gradient Tree Boosting, kết hợp dữ liệu đa nguồn
...
var exportPath = 'users/' + GEE_USERNAME + '/gtb-biomass/';
...
Export.image.toAsset({
    ...
    description: 'gtb-S2_Composite',
    assetId: exportPath + 'gtb-s2_composite',
    region: geometry,
    scale: 100,
    crs: 'EPSG:4326',
    maxPixels: 1e10
...
Export.image.toAsset({
    ...
    description: 'gtb-DEM_Bands',
    assetId: exportPath + 'gtb-dem_bands',
    region: geometry,
    scale: 100,
    crs: 'EPSG:4326',
    maxPixels: 1e10
...
Export.image.toAsset({
    ...
    description: 'gtb-GEDI_Mosaic',
    assetId: exportPath + 'gtb-gedi_mosaic',
    region: geometry,
    scale: 100,
    crs: 'EPSG:4326',
    maxPixels: 1e10
...
Export.image.toAsset({
    ...
    description: 'gtb-GEDI_Canopy_Height',
    assetId: exportPath + 'gtb-gedi_canopy_height',
    region: geometry,
    scale: 100,
    crs: 'EPSG:4326',
    maxPixels: 1e10
...
// Hàm chuyển đổi từ linear sang dB
function toDB(image) {
    return image.log10().multiply(10.0);
}

// Lọc ảnh Sentinel-1 theo thời gian, khu vực và các thông số khác
...
// Áp dụng các bước tiền xử lý cho Sentinel-1
var s1Processed = s1.map(function (image) {
    var EdgeMask = image.select('angle');
    var vv = image.select('VV').convolve(ee.Kernel.gaussian(30, 20, 'meters')); // Làm mịn ảnh VV
    var vh = image.select('VH').convolve(ee.Kernel.gaussian(30, 20, 'meters')); // Làm mịn ảnh VH
    var s1_image = ee.Image.cat(vv, vh).rename(['VV', 'VH']);

    // Sử dụng DEM đã có (glo30Filtered.mosaic()) để hiệu chỉnh địa hình
    var demForS1 = glo30Filtered.mosaic().setDefaultProjection(demProj).clip(geometry);

    return s1_image
        .unitScale(-25, 0) // Chuyển đổi sang thang đo phù hợp trước khi chuyển sang dB nếu cần
        .copyProperties(image, ["system:time_start"]);
});


// Tạo ảnh composite Sentinel-1 và cắt theo geometry
var s1Composite = s1Processed.median()
    .setDefaultProjection(s2Projection) // Sử dụng chung hệ chiếu với S2 để dễ kết hợp
    .clip(geometry);

// Hiển thị các kênh Sentinel-1
...
// Thêm export cho S1 Composite
Export.image.toAsset({
    image: s1Composite.clip(geometry).reproject('EPSG:4326', null, 100),
    description: 'gtb-S1_Composite',
    assetId: exportPath + 'gtb-s1_composite',
    region: geometry,
    scale: 100,
    crs: 'EPSG:4326',
    maxPixels: 1e10
...
// ------------------------------
// PHẦN 5: XÂY DỰNG VÀ HUẤN LUYỆN MÔ HÌNH GRADIENT TREE BOOSTING
// ------------------------------

// Nạp các asset đã xuất để chuẩn bị cho quá trình xây dựng mô hình
var s2Composite_asset = ee.Image(exportPath + 'gtb-s2_composite');
var demBands_asset = ee.Image(exportPath + 'gtb-dem_bands');
var gediMosaic_asset = ee.Image(exportPath + 'gtb-gedi_mosaic');
var canopyHeight_asset = ee.Image(exportPath + 'gtb-gedi_canopy_height');
var s1Composite_asset = ee.Image(exportPath + 'gtb-s1_composite');
...
// Huấn luyện mô hình Gradient Tree Boosting
var model = ee.Classifier.smileGradientTreeBoost(100)  // Sử dụng 100 cây quyết định
...
Export.image.toAsset({
    ...
    description: 'gtb-Predicted_Image',
    assetId: exportPath + 'gtb-predicted_agbd',
    region: geometry,
    scale: gridScale,
    crs: 'EPSG:4326',
    maxPixels: 1e10
...
var s2Composite_for_agb = ee.Image(exportPath + 'gtb-s2_composite');
var predictedImage_for_agb = ee.Image(exportPath + 'gtb-predicted_agbd');
...
// Trích xuất thông tin độ quan trọng của các biến từ mô hình Gradient Tree Boosting
...
var scriptNameVal = 'gtb';              // Tên script
var modelTypeVal = 'GradientTreeBoost';     // Loại mô hình sử dụng
...
summaryFeaturesList = summaryFeaturesList.add(ee.Feature(null, {
    'script_name': scriptNameVal, 'model_type': modelTypeVal, 'dataset_type': datasetTypeVal,
    'category': 'DataSource_Parameters', 'item': 'PredictedAGBD_AssetID', 'value': exportPath + 'new-gtb-predicted_agbd'
}));
summaryFeaturesList = summaryFeaturesList.add(ee.Feature(null, {
    'script_name': scriptNameVal, 'model_type': modelTypeVal, 'dataset_type': datasetTypeVal,
