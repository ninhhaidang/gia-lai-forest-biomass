...
// ------------------------------
var exportPath = 'users/bonglantrungmuoi/rf-full-data/';
Export.image.toAsset({
    image: s2Composite.clip(geometry),
    description: 'rf-full-data-S2_Composite_Export',
    assetId: exportPath + 'rf-full-data-s2_composite',
    region: geometry,
    scale: 100,
    maxPixels: 1e10
});
...
Export.image.toAsset({
    ...
    description: 'rf-full-data-DEM_Bands_Export',
    assetId: exportPath + 'rf-full-data-dem_bands',
    region: geometry,
    scale: 100,
    maxPixels: 1e10
});
...
Export.image.toAsset({
    ...
    description: 'rf-full-data-GEDI_Mosaic_Export',
    assetId: exportPath + 'rf-full-data-gedi_mosaic',
    region: geometry,
    scale: 100,
    maxPixels: 1e10
});
...
Export.image.toAsset({
    ...
    description: 'rf-full-data-GEDI_Canopy_Height_Export',
    assetId: exportPath + 'rf-full-data-gedi_canopy_height',
    region: geometry,
    scale: 100,
    maxPixels: 1e10
});
...
var s2Composite_asset = ee.Image(exportPath + 'rf-full-data-s2_composite');
var demBands_asset = ee.Image(exportPath + 'rf-full-data-dem_bands');
var gediMosaic_asset = ee.Image(exportPath + 'rf-full-data-gedi_mosaic');
var canopyHeight_asset = ee.Image(exportPath + 'rf-full-data-gedi_canopy_height');
var gridScale = 100;
...
// Cập nhật danh sách các đặc trưng (predictors) bao gồm canopy height
var predictors = s2Composite_asset.bandNames().cat(demBands_asset.bandNames()).cat(canopyHeight_asset.bandNames());
var response = gediMosaic_asset.bandNames().get(0); // Đổi tên biến 'predicted' thành 'response'
...
var responseImage = stackedResampled.select([response]); // Sử dụng biến 'response'
...
var model = ee.Classifier.smileRandomForest(50)
    ...
    .train({
        features: training,
        classProperty: response, // Sử dụng biến 'response'
        inputProperties: predictors
    });

// Dự đoán trên tập huấn luyện để tính RMSE
var trainingPredictions = training.classify({ // Đổi tên biến 'predicted' thành 'trainingPredictions'
    classifier: model,
...
var calculateRmse = function (input) {
    var observed = ee.Array(input.aggregate_array(response)); // Sử dụng biến 'response'
    var predictions = ee.Array(input.aggregate_array('agbd_predicted')); // Đổi tên biến 'predicted' thành 'predictions'
    var rmse = observed.subtract(predictions).pow(2)
        .reduce('mean', [0]).sqrt().get([0]);
    return rmse;
};
var rmse = calculateRmse(trainingPredictions); // Sử dụng 'trainingPredictions'
print('RMSE', rmse);
...
var predictedImage = stackedResampled.classify({
    classifier: model,
    outputName: response // Sử dụng biến 'response' cho outputName nếu muốn tên band giống nhau
});

// Xuất hình ảnh dự đoán
Export.image.toAsset({
    image: predictedImage.clip(geometry),
    description: 'rf-full-data-Predicted_Image_Export',
    assetId: exportPath + 'rf-full-data-predicted_agbd',
    region: geometry,
    scale: gridScale,
    maxPixels: 1e10
});
...
// ------------------------------
var s2Composite_for_agb = ee.Image(exportPath + 'rf-full-data-s2_composite');
var predictedImage_for_agb = ee.Image(exportPath + 'rf-full-data-predicted_agbd');
...
var totalAgb = stats.getNumber(response); // Sử dụng biến 'response' nếu tên band là 'agbd'
...
// Metadata
var scriptNameVal = 'rf-full-data'; // JS string
